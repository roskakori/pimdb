...
1. The dataset model as created by :command:`pimdb transfer`.
2. The normalized model as created by :command:`pimdb build` and derived from
...
they do, especially :file:`names.basic.tsv.gz`.

You can find a short description of the datasets and the available fields at
the page about the `IMDb datasets <https://www.imdb.com/interfaces/>`_.
...
2. Without foreign key relations is easily possible to
   :command:`pimdb transfer` multiple tables in parallel.

Here is an example query that lists all the titles directed by Alan Smithee:

.. literalinclude:: examples/titles_directed_by_alan_smithee.sql
    :language: sql
    :lines: 2-
    :caption: Example query: titles directed by Alan Smithee
...
might slow down future runs of :command:`pimdb transfer`. Also they take more
...
have several issues that normalized tables created with :command:`pimdb build`
solve:

#. 1:N relations are stored in relation tables instead of comma separated
   ``varchar`` fields. For exaple, compare ``NameBasics.knownForTitles``
   with ``name_to_known_for_title``.
#. Inconsistent or non-existent references are removed and replaced by clean
   foreign key relationships.
#. SQL joins can be performed more efficiently using integer ``id`` fields
...
#. Ratings from ``TitleRatings`` (dataset: ``title.ratings``) have been
...
.. figure:: images/name.png
    ...
    The essential data model for table ``name``
...
.. figure:: images/title.png
    ...
    The essential data model for table ``title``

As example here is a query to list the genres of a certain title:

.. literalinclude:: examples/genres_for_wyrmwood.sql
    :language: sql
    :lines: 2-
    :caption: Example query: genres of "Wyrmwood: Road if the Dead"
...
.. figure:: images/title_alias.png

    The essential data model for table ``title_alias``

And finally names and titles can be related to each other. A simple variant are
the titles a person is known for:

.. figure:: images/name_to_known_for_title.png

    The data model for known titles of a name.

As example, here is a query that lists the titles Alan Smithee is known for:

.. literalinclude:: examples/titles_alan_smithee_is_known_for.sql
    :language: sql
    :lines: 2-
    :caption: Example query: titles Alan Smithee is known for
...
.. figure:: images/participation.png

    The data model about who participated in which title and possibly played
    what character.
...
.. literalinclude:: examples/titles_with_a_jamed_bond_character.sql
    :language: sql
    :lines: 2-
    :caption: Example query: movies with a character named "James Bond" and the respective actor
